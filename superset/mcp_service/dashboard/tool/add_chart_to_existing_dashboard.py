...
# Unicode blocks stripped from tab text before matching.  The blocks are
# matched by code point instead of through a regex character class: astral
# plane endpoints written inside a class are ambiguous and can be read as
# overlapping surrogate ranges.
...
def _strip_emoji(text: str) -> str:
    """Remove characters belonging to the emoji Unicode blocks."""
    return "".join(
        char
        for char in text
        if not any(start <= ord(char) <= end for start, end in _EMOJI_CODE_POINT_RANGES)
    )
